...
def Repo():
    if os.path.exists(os.path.join(xbmc.translatePath("special://home/addons/").decode("utf-8"), 'Zona.tv')):
        return
        
    url = "https://github.com/Torokodi/Zona.tv/tree/master/repo"
...
addon = xbmcaddon.Addon('plugin.video.Zona.tv')
...
FANART = "https://raw.githubusercontent.com/Andorth/hercules/master/imagenes/fanart.gif"
...
else: SOURCES = [{"url": "http://bit.ly/2HOrt2P", "fanart": "https://espial.com/wp-content/uploads/2015/03/Espial-Wins-3rd-Major-Cable-Operator-for-RDK-and-HTML5-solution.jpg", "title": "Zona TV"}]

def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.Zona.tv-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Zona.tv,We failed with error code - "+str(e.code)+",10000,"+icon+")")
				
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Zona.tv,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Zona.tv,New source added.,5000,"+icon+")")
        if not url is None:
...
def playmediawithproxy(media_url, name, iconImage,proxyip,port, proxyuser=None, proxypass=None): #jairox
    ...
    if media_url==None or media_url=='':
        xbmc.executebuiltin("XBMC.Notification(Zona.tv,Unable to play empty Url,5000,"+icon+")")
        return
        ...
        try:
            while player.is_active:
                ...
                if player.urlplayed==False and time.time()-beforestart>12:
                    print 'failed!!!'
                    xbmc.executebuiltin("XBMC.Notification(Zona.tv,Unable to play check proxy,5000,"+icon+")")
                    break
                #xbmc.sleep(1000)
        except: pass
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(Zona.tv,Urlresolver no soporta este dominio. - ,5000)")
        resolver=url
    return resolver
...
def download_file(name, url):
        ...
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('Zona.tv','Elije una ruta para la descarga ',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('Zona.tv', 'Quiere añadir este archivo a tu dispositivo?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            if len(parentalblockedpin)>0:
                if parentalblock:
                    contextMenu.append(('Activar licencia Zona.tv','XBMC.RunPlugin(%s?mode=55&name=%s)' %(sys.argv[0], urllib.quote_plus(name))))
                else:
                    contextMenu.append(('resetear  Zona.tv','XBMC.RunPlugin(%s?mode=56&name=%s)' %(sys.argv[0], urllib.quote_plus(name))))
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Borrar de Favoritos Zona.tv','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Agregar a Favoritos de Zona.tv','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#        print 'parentalblockedpin',parentalblockedpin
        if len(parentalblockedpin)>0:
            if parentalblock:
                ...
                contextMenu.append(('Desactivar Zona.tv','XBMC.RunPlugin(%s?mode=56&name=%s)' %(sys.argv[0], urllib.quote_plus(name))))
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Borrado de Favortitos de Zona.tv','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Agregar a favoritos de Zona.tv','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
        try:
...
if mode==None:	
    ...
    import xbmc
    #xbmc.executebuiltin("XBMC.Notification(Zona.tv, Version: 1.4.1 ,5000,"+icon+")")
    ...
    line1 = "Zona TV:"
    line2 = "Este addon es gratuito, nosotros no alojamos, ni contenemos contenido de terceros en nuestro addon. Todo este software esta hecho con motivos de investigacion y estudio. Este no puede utilizarse para fines lucrativos."
    line3 = "Solo mostramos el contenido de web publicas de internet. Asi mismo no somos responsables de dicho contenido. Que es porpiedad intelectual de sus respectivos dueños. Todo el contenido es gratuito y de libre distribucion."
...
#            ln+='</item>'
        ...
        if not (regexs and 'notplayable' in regexs and not url):        
            if url:
                ...
                xbmc.executebuiltin("XBMC.Notification(Zona.tv,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(Zona.tv,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(Zona.tv, licencia activa,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(Zona.tv, Licencia no activa! ,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(Zona.tv, no activo,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
